...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <D.Close className="absolute right-4 top-4 rounded-md p-1 opacity-70 hover:opacity-100 transition-opacity">
        <X className="h-4 w-4" />
      </D.Close>
    </D.Content>
  </DialogPortal>
...
>(({ className, ...props }, ref) => (
  <D.Title ref={ref} className={cn("text-lg font-semibold leading-none tracking-tight", className)} {...props} />
));
