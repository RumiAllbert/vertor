...
export function Showcase() {
  ...
  // RAF-based scroll tracking — works in real browsers; the dev preview's
  // headless renderer doesn't fire scroll/IO callbacks, but production does.
  React.useEffect(() => {
    let raf = 0;
  ...
  return (
    ...
    >
      {/* Sticky stage — fills nearly the whole viewport */}
      <div className="sticky top-0 z-0 flex h-dvh w-full flex-col items-center justify-center px-4 md:px-8">
        ...
        {/* Caption: positioned under the mockup, fades between chapters */}
        <div className="relative mt-8 hidden h-[110px] w-full max-w-[1500px] md:block">
          {CHAPTERS.map((c) => (
            <div
              key={c.id}
              aria-hidden={frame !== c.id}
              className={cn(
                "absolute inset-0 grid grid-cols-[auto_1fr_auto] items-baseline gap-8 transition-opacity duration-500",
                frame === c.id ? "opacity-100" : "pointer-events-none opacity-0",
              )}
            >
              <div className="small-caps text-ink">{c.eyebrow}</div>
              <div>
                <h3 className="display text-[28px] italic leading-[1.05] md:text-[34px]">
                  {c.title}
                </h3>
      ...
      {/* Mobile captions (each chapter is its own viewport-tall section) */}
      ...
      {/* Desktop scroll spacers — invisible, just drive section height */}
...
const SOURCE_LINE_1 = "Le traducteur est un écrivain qui écrit dans la langue d'autrui.";
const SOURCE_LINE_2 = " Il habite, le temps d'un livre, le souffle d'une voix qui n'est pas la sienne, et la rend audible.";
const SOURCE_PARA_2 = "C'est un art discret, presque effacé, et pourtant rien ne s'écrit sans lui.";

const TRANS_LINE_1 = "A translator is a writer who writes in the language of another. They inhabit, for the length of a book, the ";
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-lg border border-hairline bg-background shadow-[0_50px_120px_-40px_rgb(0_0_0/0.35)]">
      ...
      <div className="flex items-center gap-3 border-b border-hairline px-6 py-3 text-[12.5px]">
        ...
        <span className="ml-4 inline-flex items-center gap-2 rounded-sm border border-foreground bg-foreground px-3 py-1 font-medium text-background shadow-[2px_2px_0_var(--ink)] transition-all duration-500">
          <span
            className={cn(
              "h-1 w-1 rounded-full transition-colors",
              frame === "translate" ? "bg-background caret" : "bg-ink",
            )}
          />
          <span className={frame === "translate" ? "shiny" : ""}>
      ...
      <div className="grid grid-cols-2 gap-px bg-hairline">
        {/* SOURCE PANE */}
        <div className="relative bg-background px-8 py-6 min-h-[400px]">
          <div className="small-caps mb-4">Source</div>
          <div className="space-y-4 font-serif text-[14.5px] leading-relaxed">
            <p>
              <span className="reveal-text inline" data-revealed>
                {SOURCE_LINE_1}
              </span>
              <span className="reveal-text inline" data-revealed={!isCompose}>
                {SOURCE_LINE_2}
              </span>
              {isCompose && (
        ...
        {/* TRANSLATION PANE */}
        <div className="relative bg-background px-8 py-6 min-h-[400px]">
          <div className="small-caps mb-4">Translation</div>

          {/* Empty-state quote, cross-fades out as soon as user moves past compose */}
          <div
            className="cross-fade absolute inset-x-8 top-1/2 -translate-y-1/2 text-center"
            data-visible={isCompose}
          >
            <p className="display max-w-[24ch] mx-auto text-[18px] italic text-muted-foreground">
              &ldquo;Translation is that which transforms everything so that nothing changes.&rdquo;
            </p>
          </div>

          {/* The actual translation content fades in over the quote */}
          <div
            className="cross-fade space-y-4 font-serif text-[14.5px] leading-relaxed"
            data-visible={isTranslateOrLater}
          >
            <p>
              <span className="reveal-text inline" data-revealed={isTranslateOrLater}>
                {TRANS_LINE_1}
                <span className="word-highlight" data-active={isRefine}>breath</span>
                {TRANS_LINE_1_TAIL}
              </span>
              {frame === "translate" && (
            ...
            <p className="reveal-text" data-revealed={isRefine}>
              {TRANS_PARA_2}
            </p>
          </div>

          {/* Variations popover, slides in only on refine */}
          <div
            className="slide-in absolute right-6 top-28 w-[280px] origin-top rounded-md border border-foreground/15 bg-background p-0 text-[12px] shadow-[0_18px_42px_-12px_rgb(0_0_0/0.32)]"
            data-visible={isRefine}
          >
            ...
            <div className="border-t border-hairline">
              {["breath", "pulse", "cadence"].map((v, i) => (
                ...
                >
                  ...
                  <span className="font-serif text-[13px] text-foreground">{v}</span>
                </div>
              ))}
            </div>
          </div>
