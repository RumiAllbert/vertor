...
export function Hero({ authEnabled }: { authEnabled: boolean }) {
  ...
  return (
    ...
    >
      {/* Aurora — fades out as the hero scrolls away. Always visible from
          the first paint, including during the intro's aurora-only opening. */}
      <div
        aria-hidden
      ...
      >
        <h1 className="intro-wordmark display wordmark-crisp text-[22vw] leading-[0.85] tracking-[-0.02em] md:text-[200px] md:leading-[0.85]">
          Vertor
        </h1>

        <div className="intro-cascade-1 mt-10 flex w-full max-w-[760px] items-center gap-5 px-4">
        ...
        {/* Tagline — single line so the typewriter clip-path reveals it
            cleanly. whitespace-nowrap prevents reflow on narrow screens. */}
        <p className="intro-typewriter subtext-crisp mt-6 max-w-[44ch] whitespace-nowrap text-[16px] font-medium text-foreground/85 md:text-[19px]">
          A workspace for translators, writers, and editors.
        </p>

        <div className="intro-cascade-2 mt-10 flex flex-col items-center gap-4 sm:flex-row sm:gap-7">
          <Link
            href="/app"
